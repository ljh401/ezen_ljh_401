...
public class Ex2_예외처리 {
	...
	public static void main(String[] args) {
		// p.470 : 다중 catch : 다양한 예외에 따른 서로 다른 흐름 제어를 할 수 있다!
		String[] array = {"100", "1oo"};	
		
		// Integer.parseInt(문자열인숫자) : 문자열숫자 => 정수형숫자 정수로 타입 변환
		...
		for( int i = 0; i<=array.length; i++) { // 인덱스 : 0~1 // 길이 : 2 // 인덱스 2는 없음
			...
			// 예외클래스 catch 작성 순서 : 하위 예외 클래스 블록 먼저 작성
			}
			//catch( Exception e) {System.out.println("예외발생 : " + e );} 
				// 예외 상위 클래스가 앞에 있으면 아래 있는 하위 클래스들은 실행x
			catch( NumberFormatException e) {System.out.println("숫자로 변환할 수 없는 데이터 :" + e);} 
