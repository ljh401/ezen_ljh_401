...
public class Ex1_멀티스레드 {
	...
	public static void main(String[] args) { // 메인스레드가 main메소드를 호출하면 코드 시작
		...
		Toolkit toolkit = Toolkit.getDefaultToolkit(); // java.awt : JAVAUI 제공
			// * 5번의 비프음 발생시켰는데 .. 비프음 1번 들림 [ 비프음보다 for문이 더 빠름 ]
		...
		for( int i = 0; i<5; i++) {
			...
			try {
				Thread.sleep(500);// 예외처리 발생 [ Unhandled exception type InterruptedException ]
			}catch (Exception e) {
		...
		// 1. [main스레드가 작업스레드 생성]main스레드와 작업 스레드 객체 만들기
		...
		// ------------------------------------------멀티 스레드 [구현체]----------------------------------------//		
		비프음2 비프음2 = new 비프음2(); // Runnable 인터페이스 구현한 객체 [ 구현체 ]
		thread2.start(); // Thread 클래스에 start()실행시 run 메소드 실행
...
/* 
	프로그램 : 명령문의 집합
		...
		*멀티스레드는 프로그램 *내부*에서의 멀티 태스킹
		...
		- 멀티스레드에서는 하나의 스레드가 오류 발생하면 프로세스 종료되므로 영향을 미친다.
			- 카카오톡( 프로세스[채팅기능/스레드,첨부파일전송/스레드] ) 
				파일 전송 스레드에서 오류가 발생하면 카카오톡 프로세스 종료되므로 채팅도 같이 종료
				
	멀티 스레드 : 병렬로 데이터 처리하는 곳
		1. 사용처 : 앱 , 웹 서버 등등
		2. 메인스레드 : 모든! 자바 프로그램메인스레드가 main메소드를 실행하면서 시작된다.
		3.
			싱글스레드 : main메소드만 사용하는 경우 [ *메인스레드가 종료되면 프로세스 종료 ]
			멀티스레드 : main메소드에 새로운 작업스레드를 생성하는 경우 [*모든스레드가 종료되면 프로세스 종료 ]
		4. 작업 스레드 생성
			Thread 변수명 = new Thread( Runnable 구현객체 );
			- start() : run메소드 호출
		5. 
        ...
        6. 익명[이름없는]객체
        	익명구현체
        		클래스명 클래스 = new 클래스명() { 오버라이딩메소드 }
        			비프음2 비프음2 = new 비프음2() { public void run() {} };
        	익명구현객체
