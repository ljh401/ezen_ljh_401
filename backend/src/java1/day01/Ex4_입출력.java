...
public class Ex4_입출력 { // class s
	...
	public static void main(String[] args) { // main s
		...
		System.out.println("출력문구4");
			// 3. System.out.printf("형식/포멧 문자", 값/변수);		: 형식문자열에 맞춰 값 출력
			// [ p.69 ] d:정수 f:실수	s:문자열		
		// %자릿수d : 오른쪽 정렬		%-자릿수d : 왼쪽 정렬		%06d : 비어있는 자릿수 0채움
		// .소수점자릿수f : 소수점 자릿수만큼 출력
		int value = 123;
		System.out.printf("상품의 가격 : %d원 \n" , value ); // %d 위치에 value 변수 출력
		System.out.printf("상품의 가격 : %6d원 \n" , value );// %6d : 6칸 정수 자리에 value 변수 출력 [ 자릿수가 비어있으면 공백 처리 ]
		System.out.printf("상품의 가격 : %-6d원 \n" , value );// %6d : 6칸 정수 자리에 value 변수 출력 [ 자릿수가 비어있으면 공백 처리 ]
		...
		Scanner scanner = new Scanner( System.in);	// System.in : 시스템 입력 , System.out : 시스템 출력 = 콘솔창
			
			// 2. 입력 객체를 이용한 입력값 호출/가져오기
				// 1. 입력받은 문자열 가져오기 scanner.next();
				// 2. 변수에 저장
		String 문자열 = scanner.next();
				// scanner.next(); : 띄어쓰기포함불가능x	 	vs		scanner.nextline(); : 띄어쓰기포함가능o
				// 3. 변수에 출력 확인
