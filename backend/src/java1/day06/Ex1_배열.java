...
public class Ex1_배열 { // class s
   ...
   public static void main(String[] args) { // main s
      ...
      // 4. 삭제 ( 없음 : 배열은 선언될 때 고정!!) * 해당 인데스를 null로 만든다.
      season[1] = null;
      System.out.println( Arrays.toString( season ) );
      ...
      // 총합/ 평균
      int sum = 0;
      for(int i = 0; i<3; i++) { // i는 0부터 3미만까지 1씩증가 [ 0~2 ] // scores배열은 [ 0~2 ]
    	  sum += scores[i];
      ...
      // 2. double형 배열
	      ...
	      // 2.배열내 모든 데이터 호출 // 길이를 모르는 경우 => 배열명.length : 배열 길이 수 반환
	      										// 배열명.length  ,  문자열.length()
	      for(int i = 0; i<arr2.length ; i++) {System.out.println(arr2[i]);}
	      			// i <= arr2.length : 0~3 오류 발생	// i<=arr2.length-1 : 0~2
	      			// i <arr2.length : 0~2
	      // 3. 배열내 값 변경
	      arr2[0] = 0.1;	arr2[1] = 0.2;	 arr2[2] = 0.3;
	      // arr2.push [x] : JS에서 사용되는 함수이므로 자바 불가능
	      //arr2[3] = 0.4; // 인덱스 넘어가서 오류남
	      // 오류 이유 :  java.lang.ArrayIndexOutOfBoundsException: Index 3 out of bounds for length 3
	  	  //at java1.day06.Ex1_배열.main(Ex1_배열.java:73)
	   
	   // 3. String 클래스 배열
	      // 1. 선언
	      String[] arr3 = new String[3];	// String객체(문자열)를 3개 저장할수있는 배열 선언 [ null, null , null ]
	      ...
	      arr3[1] = null;	System.out.println(Arrays.toString(arr3));
	      	...
	      	for(int i= 1; i <arr3.length; i++ ) {
	      		// 만약에 마지막 인덱스에 도착했으면 다음 인덱스가 없으므로 당기기 할 필요 x
