...
public class Ex2_연산자 { // class s // 모든 자바 코드는 클래스에서 작성
	...
	public static void main(String[] args) { // main s // 모든 실행 코드는 main함수에서
		...
		// 2. 비교연산자 [ p.88 ] : 연산 후 반환 => boolean true / false
		...
		// 3. 논리 연산자 [ p.91 ]
		System.out.println(" x >= 10 and y >= 10 : " + (x >= 10 && y >= 10)); // 조건이 모두 true이면 true and false => false
		System.out.println(" x >= 10 or y >= 10 : " + (x >= 10 || y >= 10)); // 조건이 하나라도 true이면 true or false => true
		...
		// 6. 증감연산자
		System.out.println(" ++x : " + (++x)); // 해당 명령어에서 x 1증가 선위증가 [ 출력 보다 ]
		System.out.println(" x++ : " + (x++)); // 해당 명령어에서 x 1증가 후위증가 [ 출력 보다 ]
		System.out.println(" x : " + x);
