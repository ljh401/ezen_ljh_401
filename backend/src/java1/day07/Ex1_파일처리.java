...
public class Ex1_파일처리 { // class s
	public static void main(String[] args) throws IOException { // main s
		...
		// 스택영역 : 함수는 호출'{'시 메모리 할당 종료'}'시 메모리 초기화 *지역변수
		...
		// 파일 처리 라이브러리(미리 만들어진 클래스/메소드)
			...
			// 2. FileInputStream : 파일 불러오기 관련된 메소드 제공하는 클래스
			  new FileInputStream("파일 경로")				: 해당 파일과 연동/연결
			
			
			// 2. 제공하는 함수/메소드
				 1. .write(바이트배열)		: 해당객체와 연결된 파일의 바이트배열을 내보내기	
				 2. .read(바이트배열)		: 해당객체와 연결된 파일의 바이트 읽어와서 바이트배열에 저장
			...
			// 3. File			: 파일에 정보/제어 관련 메소드 제공
			 	new File("파일경로");
			    메소드
			    	1. .exist() 	: 파일 경로에 파일 존재여부 true /false 반환
			    	2. .delete()	: 파일 경로에 파일 삭제 실행
			    	3. .getPath()	: 파일에 위치한 경로 반환
				 	4. .length() 	: 
		*/
		...
		byte[] 복권파일바이트배열 = new byte[(int)file.length()]; // 용량 만큼 배열 선언
			// 3. 읽어오기
		복권파일.read(복권파일바이트배열);
			// 4. 바이트배열 --> 문자열 반환
		...
		FileInputStream 관광지파일 = new FileInputStream(경로);
			// 2. 파일 정보 객체 생성 [ file.length() 메소드 사용하기 위해 ]
		...
		//String 관광지파일정보 = new String(관광지파일바이트배열 , "UTF-8"); // 한글 존재시 UTF-8로 반환
		String 관광지파일정보 = new String(관광지파일바이트배열 , "EUC-KR"); // 한글 존재시 EUC-KR로 반환
