	...
	public static void main(String[] args) { 
		...
		while( true ) { // 무한루프 [ 종료조건 : 4 입력시 break; ] 
			...
			if( ch == 1) {
				...
				if(account.equals("123-1234")) {
					System.out.print(">>> 예금할 금액 : ");
					balance += scanner.nextInt();
					System.out.print(">>> 예금 후 금액 : " + balance);
				} else { System.out.println(">>> 없는 계좌번호입니다1.");}
			} else if( ch == 2 ) { 
	            System.out.print(">>> 출금할 계좌번호 : "); 
	            String 계좌입력 = scanner.next();
	            
	            if(계좌입력.equals( account )) {
	               System.out.print(">>> 계좌 비밀번호 : " );
	               short 비번입력 = scanner.nextShort();
	                  
	               if(비번입력 == password ) {
	                  System.out.print(">>> 출금할 금액 : ");
	                  int 출금입력 = scanner.nextInt();
	                  
	                     if(balance<출금입력) {
	                        System.out.println(">>> 잔액부족 : " + balance);
	                     } else { System.out.println(">>> 출금후 금액 : " + (balance-=출금입력)); }
