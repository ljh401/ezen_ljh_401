      ...
      while(true) { // 프로그램 무한루프 
         ...
         // 2. 재고관리 배열에 있는 제품 정보 만 출력 가능 하도록
            for( int i = 0 ; i<재고관리.length ; i++ ) { // 배열 하나씩 출력
            	if(!재고관리[i].equals("")) { // 만약에 배열내 공백이 아니면 // 제품 존재
         ...
         /* ---------------------------------선택한 제품을 바구니 담기----------------------------------*/
        	 ...
        	 // 4. 파일에 내보내기 // 파일내 제품들의 변화가 있으므로 업데이트 // 현재 재고관배열 상태를 파일에 저장
        	 String outStr = "";
        	 ...
        	 FileOutputStream fileOutputStream2 = new FileOutputStream(filePath); // 새로쓰기
