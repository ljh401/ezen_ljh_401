	...
	데이터베이스 : 데이터[자료] , 베이스[모임]
        ...
        - 용어
				DBA[ 데이터베이스 관리자 ]
					- 개발자
				DBMS[ 데이터베이스 관리 시스템 ]
					- Mysql , oracle(1등) , mariaDB , SQLSERVER - 각 회사별 소프트웨어
                    - SQL [ 구조화된 질의 언어 ]
                DB[데이터베이스]
		...
		- 관계
			...
			-SQL : DB 조작/관리/제어 할때 사용되는 언어/문법 (대소문자 구분X)
				- 1. DDL [ 데이터베이스 정의어 ]
                    ...
                    - 6. show		:
						- show databases				 : 데이터 베이스 전체 목록 보기
                        - show variables like 'datadir'; : 데이터 베이스 저장소 로컬 경로 확인
					- 7. use
						- use db명 	: 데이터 베이스 사용
                - 2. DML [ 데이터베이스 조작어 ]
			...
			- 테이블 용어
				테이블 [ 표 ]
					...
					속성		column	 열				필드
					 |		  |		 |				 |
					학번		학생명	전화번호			주소
                    1010	이진형	010-1111-1111	안산		- 가로 = 행 = row = 레코드
                    1011	황태웅	010-1111-1111	제주		- 2번 레코드
                    
				1. 컬럼 = column = 열 = 필드 = 속성 : 테이블[표]의 열
					학번필드 , 학생명필드 , 전화번호필드 , 주소필드
                    
				2. row = 행 = 레코드 : 테이블[표]의 행
					1번레코드 , 2번레코드 , 3번레코드
				
                3. 행/열로 구성된 테이블(표)를 다른 표와 연결/관계 = 관계형 데이터베이스
*/

#예1 : 데이터베이스( 여러개의 테이블(표)들이 저장할 수 있는 공간 ) 생성
create database sqldb1;
	# create : 생성한다.
    # database : 데이터베이스.
    # sqldb1 : 이름 정의[ 아무거나 ,대소문자 구분X, 키워드X ]
    # ;		 : 명령어 끝마침.
...
#예5 : 만약에 데이터베이스 존재했을때 삭제 [ *만약에 데이터베이스 없을때 삭제명령어 사용하면 오류 발생!! ]
...
/*-------------------테이블(표) 만들기---------------------*/
# 테이블(표)는 데이터베이스 안에 저장 [ * 무슨 데이터베이스 생성할건지...use DB명; ]
#예1 : 테이블 생성
