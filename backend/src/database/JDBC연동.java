...
public class JDBC연동 {
	...
	public static void main(String[] args) {
		...
		//--------------------------------------예외가 발생할 것 같은 코드들 try {} 안에-------------------------//
		// 1. p.908 : 현재 프로젝트에 JDBC 파일을 build path add 하기
		// 2. p.909 : DB연결하기 위해 JDBC Driver 클래스 찾아서 메모리 로딩하기
			// Class.forName("oracle.jdbc.OracleDriver");
		
		Class.forName("com.mysql.cj.jdbc.Driver");		// mysql 기준
			// 만약에 해당 JDBC 드라이버 클래스가 검색이 실패하면 예외발생 -> 예외처리
		
		// 3. DB 연결 [ DriverManager.getConnection() 메소드가 연결 성공하면 DB와 연결된 객체 리턴 -> 만약에 실패하면 예외 발생 -> 예외처리]
...
/*
	예외처리 : 
		...
		- try{} catch{}
			...
			* try{}안에서 예외가 발생하는 순간 바로 catch로 이동하기 때문에 예외발생한 코드 아래로는 실행 안됨!
			
		- 예외[오류]가 발생할 것 같은 코드
			- 일반예외 : 컴파일 과정에서 체크	 	[ - 컴파일 체크하기 때문에 미리 처리가능 ] 
				- 파일처리 , DB연동 , 입출력 등등
