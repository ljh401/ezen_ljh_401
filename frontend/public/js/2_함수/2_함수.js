    ...
    함수   [ 함 : 상자   /  수 : 숫자 ]
      ...
      3. 사용 목적
         ...
         4. 함수 선언
         	...
         	3. ( ) : 인수[함수 안으로 들어가는 변수/코드/배열/객체 등]
         			, 로 구분
         	4. { } : '{' 함수 시작 / '}' 함수 끝
         	5. return : 함수 종료/리턴		[*해당 함수를 호출했던 곳으로 반환/리턴]
         				return 3;	함수를 호출했던 곳으로
         				return;		반환값이 없으므로 반환값없이 단순 함수 종료
         	
         	ex)
         		1. 유재석이 핸드폰이 고장나서 서비스 센터 간다.
 ...
 */
// 함수 정의
function 함수1(){} 					// 1. 인수없다. / 반환없다.
function 함수2( x ){} 				// 2. 인수(x = 임의의 인수 / 매개변수명) / 반환없다.	
function 함수3(x,y,z){} 				// 3. 인수 다수(, 구분) / 반환없다.
function 함수4(x,y,z){return 3;} 	// 4. 인수 있다. / 반환 값 없다. -> 함수 종료 역할
function 함수5(){return 5;} 			// 5. 인수 없다. / 반환 값 있다. -> 함수 종료 역할
function 함수6(){return;}			// 6. 인수 없다./ 반환 값 = 단순 함수 종료 역할
	// * 함수가 종료되는 조건 : 1. '}' 끝났을때 2. return 키워드를 만났을때 [아래 코드 있어도 종료!]
	
//2. 함수 호출
console.log('안녕!!')				// 1. 미리 정의된(JS 만든 사람) log(매개변수 = 출력하고 싶은 문자열)함수
배열명.splice(0,1)					// 2. 미리 정의된(JS 만든 사람) splice(인수1 = ??, 인수2 = ??)
document.querySelector('')			// 3. 미리 정의된(JS 만든사 람) querySelector(인수1 = ?? ,인수2 = ??)
...
let 함수결과1 =함수4(3, 5, 20)			// 7. 함수 종료 후 리턴/반환 값을 변수에 저장
let 함수결과2 =함수5()					// 8. 함수 종료 후 리턴/반환 값을 변수에 저장
함수6() 								// 9. 함수 종료 후 리턴/반환 값이 없으므로 별도로 저장X
