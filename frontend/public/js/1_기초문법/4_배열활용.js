...
// 1. 배열 선언 : '학생리스트' 배열명으로 비어 있는 배열 선언
...
function 추가함수(){ // f start
	...
	let snameInput = document.querySelector(".sname")
		...
		// Selector : 선택자 ( id, class, 마크업 등)
			// 1. document.querySelector(".sname") : sname 이라는 class명을 가진 input객체 호출
...
function 삭제함수(){ // f start
	...
	let name = snameInput.value
	//* 삭제하려면 인덱스 필요! 인덱스 몰라 --> 인덱스 찾자
	// 3. 삭제할 데이터의 인덱스(저장된 순번)찾자
	let index = 학생리스트.indexOf(name)
		// 해당 데이터가 배열에 존재하면 0~찾을 번호 알려주고 없으면 -1
	// 4. 배열내 인덱스를 이용한 데이터 삭제
	//* 유효성 검사 : 데이터가 개발자 원하는 데이터인지 검사
