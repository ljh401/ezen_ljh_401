...
/*
	1. 서버 소켓 만들기
		1. 클래스 선언
		2. 클래스 위에 @ServerEndpoint("서버소켓의URL정의") : 클라이언트 소켓들이 접속하는 위치
		3. 서버웹소켓의 필수 메소드
...
public class ChattingSocket {
	...
	// 1. // 클라이언트가 서버소켓에 접속했을때 매핑/연결 ( JS에서 new WebSocket 객체 생성될때 . 문제없이 접속되면 )
	@OnOpen 
	...
	public void onMessage(Session session, String msg){
		...
		접속명단.forEach(s->{
			...
			// s : 접속명단에 있는 클라이언트소켓 중 하나
				// getBasicRemote() : 메시지 전송을 제공
					// .sendText : 메시지를 String타입으로 전송 [!예외처리필수]
		});
	
		// 클라이어느 소켓으로부터 메시지를 받았을때
	}
	// 4.
