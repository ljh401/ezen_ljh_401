...
/*
 	...
 	- 저장소
 		1. JAVA/JS 메모리
 			- JAVA 종료되거나 JS 종료/페이지전환 되면 휘발성/초기화/사라짐
 		...
 		2. DB 메모리
 			- 영구 저장 [ 논리 구조(관계형/표) 저장 ]
 			- 회원정보 , 게시판정보 , 제품정보 등등
 			
 		3. 파일
 			- 영구 저장 [ 문자열 저장 ]
 			- 로그 / 기록 ( 예외처리 오류 정보 , 접속 수 , 최상위 보안 자료 )
 			- 실제 데이터
  	...
  	로그인 했다는 증거 => 인증 [ 인증서 -> 세션 저장 ]
  		...
  		JAVA(서블릿) : 
  			...
  			세션호출 : request.getSession().getAttribute( "속성명", 데이터 );
  			세션내 속성초기화
  				방법1 : (세션의 모든 속성) 초기화 하는 함수
