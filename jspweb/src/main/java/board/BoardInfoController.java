...
public class BoardInfoController extends HttpServlet {
    ...
    public BoardInfoController() { super();}
    // type : 1. 전체조회 , 2.개별조회 
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if( type.equals("1") ) { // 전체 조회 로직 
			...
			// -------------------------5. 페이지 번호 -----------------------------//
				// 5개씩		: 1페이지( 1, 5) 6페이지 (6 ,10) 11페이지(11 ,15) 
				// 10개씩		: 1~10페이지(1,10) 11~20페이지 (11,20) 21~30페이지(21,30)
				// 15개씩		: 1~15(1,15) 16~30( 16,30)
				/*
						페이지	시작		마지막		시작계산식 [ int btnsize = 5; ]
						1페이지	1		5			page*btnsize	1/5*5 => 0*5 0+1 1		
						2		1		5			page*btnsize	2/5*5 => 0*5 0+1 1
 						3		1		5			page*btnsize	3/5*5 => 0*5 0+1 1
						4		1		5			page*btnsize	4/5*5 => 0*5 0+1 1
						5		1		5			page*btnsize	5/5*5 => 1*5 5+1 6
						6		6		10			page*btnsize	6/5*5 => 1*5 5+1 6
						7		6		10
						8		6		10
						9		6		10
						10		6		10
			...
			int endbtn = startbtn+(btnsize-1);
				...
				// 만약에 마지막번호가 총 페이지수보다 크면 총 페이지수로 제한두기
	...
	protected void doDelete(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 1. 요청 
		int bno = Integer.parseInt( request.getParameter("bno") );
			// 레코드 삭제 전에 파일 이름 꺼내두기 [ 삭제 후 파일 이름 호출이 불가능 하니까 ]
		String filename = BoardDao.getInstance().getBoard(bno).getBfile();
		// 2. DAO
