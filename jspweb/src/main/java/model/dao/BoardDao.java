	...
	public ArrayList<BoardDto> getList(int bcno, int listsize, int startrow, String key, String keyword){
		...
		try {
			...
			if(!key.isEmpty() && !keyword.isEmpty()) {
				...
				// 만약에 카테고리 내 검색이면 [ 이미 where구문이 존재하기 때문에 and 조건 추가 ]
				if(bcno != 0) sql += " and ";
			...
			while(rs.next()) {
				BoardDto dto = new BoardDto(
						rs.getInt("bno"),rs.getString("btitle"), rs.getString("bcontent"), rs.getString("bfile"),
						rs.getString("bdate"), rs.getInt("bview"), rs.getInt("mno"),
						rs.getInt("bcno"), rs.getString("mid"), rs.getString("bcname"),
						rs.getString("mimg"));
				list.add(dto);
			}
		} catch (Exception e) {
