// map 쓰는 이유 : 이미지와 pk 두개 가져올라고 그러는거임.
	...
	자바스크립트 객체
		형태 : let 변수명 = { 속성명: 데이터 , 속성명: 데이터 ,속성명: 데이터 }
			...
			호출 : 객체명.속성명 또는 객체명['속성명']
				- Object.keys(객체명) : 객ㅔ 내 모든 키를 배열로 반환
				- Object.values(객체명) : 객체내 모든 데이터를 배열로 반환
				
				// * 대표 이미지 출력하기 [ 첫번째 이미지 ] / Object.keys(객체명) : 해당 객체내 모든 속성명/필드명/키 리스트로 반환
					...
					console.log(p.imgList[Object.keys(p.imgList)[0]]);	// 제품별 이미지 객체의 속성명 리스트 중에 첫번째 속성명의 사진 이름
