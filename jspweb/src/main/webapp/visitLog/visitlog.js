...
function vwrite(){
	...
	// 2. 객체화
		// let 변수명 = { 속성명 : 데이터 , 속성명 : 데이터 , 속성명 : 데이터 }
	let info = {
...
vread();
function vread(){ // 실행조건 : JS열릴때 1번 실행 , 등록성공 , 수정성공  ,삭제성공  => 최신화[화면 새로고침]
	$.ajax({
		url : "/jspweb/VisitLogController",
		method : "get",
		data : "",
		...
		let html = ``; // `백틱 [ 백틱 안에 JS문법 사용 ${}]
		
			// 리스트내 모든 데이터를
			for(let i = 0; i<r.length; i++) {
				// 객체를 HTML형식으로 누적 더하기
				html += `<div class = "visitbox"> <!-- 방문록 1개의 표시 구역 -->
...
function vupdate(vno){
	...
	// 1. 수정할 내용 입력
	let vcontent = prompt('수정할 방문록 내용 :	');			// alert() : 확인알림창 confirm() : 확인/취소알림창 prompt 
	// 2. 비밀번호가 일치할 경우에도 수정하므로 확인용 비밀번호 입력받기
	let vpwd = prompt("방문록 비밀번호 : ");
	
	// 수정 준비물 : vno(누구를) , vcontent(어떤내용으로) , vpwd(조건용 : 비밀번호 일치 여부)
	
	$.ajax({
		url : "/jspweb/VisitLogController", // 통신할 백엔드(컨트롤러 주소)	
		method : "put",
...
function vdelete(vno){
	...
	// 삭제 준비물 : vno(누구를) , vpwd(조건용 : 비밀번호 일치 여부)
